...
from pyrogram.types import InputMediaDocument, Message, InlineKeyboardButton, InlineKeyboardMarkup, InputMediaPhoto
...
# Check whether the file is already being renamed or has been renamed recently
    ...
    if episode_number:
        ...
        file = message
        data1 = f" <b> <i> Anime Details:{custom_name} -S01 - EP{episode_number} - {extracted_qualities} Tamil </i></b> \n\n 📤 Uploading the  Anime file 💥💫 "

        data = f" <b> <i> Anime Details:{custom_name} -S01 - EP{episode_number} - {extracted_qualities} Tamil </i></b> \n\n 📥 Downloading the Anime file ⚡ "

        if not TARGET_CHANNEL_ID:
            await message.reply("**Error:** Target channel not set. Use /set_target to set the channel.")
            return
             
        download_msg = await client.send_message(chat_id=TARGET_CHANNEL_ID, text="📥 Preparing to Download Anime: " +custom_name)
        try:
            path = await client.download_media(message=file, file_name=file_path, progress=progress_for_pyrogram, progress_args=(data, download_msg, time.time()))
        ...
        upload_msg = await download_msg.edit(text="📤 Ready to Upload Anime :"+ custom_name)
        ph_path = None
        ...
        try:
            sent_message=await client.send_document(
                             chat_id=Config.FILE_STORE_CHANNEL,
                             document=metadata_path if _bool_metadata else file_path,
                             thumb=ph_path,
                             caption=caption,
                             progress=progress_for_pyrogram,
                             progress_args=(data1, upload_msg, time.time())
                        )            
    ...
    # Detect quality from caption
            ...
            if not quality:
                await message.reply_text("Please send File with quality (480p, 720p, 1080p) in the File name.")
                return
    ...
    # Send or edit the post in the target channel
            ...
            if len(EPISODE_LINKS[episode_number]) == 3:
